...
Stopwords <- c("can", "actually", "weve", "also", "whats", "youre", "think", "tell", "yes", "want", "well", "bit", "get", "thats", "theyre", "thing", "ive", "able", "like", "lets", "lot", "let", "one", "will", "got", "dont", "said", "put", "done")
...
wordcloud(names(freq), freq, min.freq=225, colors=brewer.pal(6, "Dark2"), random.color = TRUE)  
```

## Visualizing each candidate's wordcloud

```{r personal-wordclouds}
# findMostFreqTerms(dtm, INDEX = rep(1 : 20, each = 1L))
#for(i in 1:2){
#   m <- as.matrix(dtm[i,])
#   freq <- colSums(m) 
