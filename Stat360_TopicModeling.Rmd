...
lda <- LDA(dtm                              #inputting our document-term-matrix
           ...
           , delta = .14                     #setting delta hyperparameter
...
This function comes from the `ldatuning` package. This function "calculates different metrics to estimate the most preferable number of topics for LDA model" [https://cran.r-project.org/web/packages/ldatuning/ldatuning.pdf]. Here, it uses 4 scoring algorithsm to evaluate the number of topics: Arun 2010, CaoJuan 2009, Griffiths 2004, and Deveaud 2014. 
...
topicStats <- FindTopicsNumber(dtm
                ...
                ,seed = 1                       #setting seed to 1
                ,delta = .1                     #setting delta hyperparameter for words over topics
                ,iter = S                       #number of trials equal to S
                ,burnin = BURNIN))              #adding burn in period
```
...
```{r posterior-lda}
beta <- posterior(lda)$terms #hyperparameter beta is the terms per topic
...
Next, we can assign pseudo-names for each topic by concatenating the 5 terms with the highest probability from each topic.

```{r pseudo-topic-names}
pseudo_name <- terms(lda, 5)
